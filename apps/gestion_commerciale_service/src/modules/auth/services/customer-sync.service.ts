...
export class CustomerSyncService {
  ...
  async syncUserWithCustomerService(userData: UserSyncData): Promise<any> {
    try {
        ...
        this.httpService.post(
          ...
          userData,
          {
            headers: {
              ...
              'X-Service-Name': 'gestion-commerciale-service',
      ...
      this.logger.error(`❌ Failed to sync user with Customer Service: ${error.message || error}`);
      throw error;
    }
  }
  ...
  async notifyUserLogin(userData: UserSyncData): Promise<boolean> {
    try {
      ...
      this.logger.warn(`Sync failed but continuing with local authentication: ${error.message || error}`);
      return false;
    }
  }
}
